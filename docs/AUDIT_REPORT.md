...
**Reviewed branch/worktree:** `agent/fix-real-video-decoding`
...
| 4 | **No automated regression gates.** The baseline package had no `lint` or `test` scripts and no CI workflow. | **Fixed.** ESLint, Vitest, Testing Library, coverage thresholds, an aggregate `check` script, and [CI configuration](../.github/workflows/ci.yml) were added. The current 128 tests cover analyzer logic, trim/crop bounds, conservative clip aggregation, clip Worker envelopes and cleanup, settings snapshots, component focus, and application flows. Real camera, rendering, and target-device paths remain open. |
...
| --- | ---: | ---: |
| `npm run build` | Pass; Vite `8.0.10`; 1,739 modules; 403 ms | Pass; Vite `8.1.5`; 1,791 modules; 263 ms |
| Main/app JavaScript | 911.61 kB raw; 249.97 kB gzip | 257.52 kB raw; 80.92 kB gzip |
| Additional current chunks | None | pose Worker: 137.62 kB raw; lazy clip workspace: 26.71 kB raw / 8.19 kB gzip; lazy `PoseScene`: 561.09 kB raw / 140.65 kB gzip |
...
| Automated tests | No script / zero repository tests | 7 files, 128 tests, all pass |
| `npm audit` | 2 high-severity vulnerable packages | 0 known vulnerabilities across the installed dependency tree |

The main/app chunk is 71.8% smaller raw and 67.6% smaller gzip, but that comparison excludes the separately loaded Worker, clip workspace, and optional 3D chunk. Build time differences are too noisy to treat as a speedup.
...
| **CI and deployment** | CI and CodeQL have passed remotely on the prior merged baseline; Vercel serves the app over HTTPS. | Require the current pull request and post-merge deployment checks to pass, verify emitted headers and camera Permissions Policy at the CDN, then run production smoke and egress tests. |
