...
Run `npm run test:e2e:install` once after a fresh checkout, then `npm run verify:all` for the same three software-gate groups CI executes. Playwright builds and verifies a fresh production bundle, then serves that exact artifact on dedicated strict port `43917`; it never reuses an existing localhost server, so a port collision fails closed instead of testing another application. The `dist` directory left after E2E is therefore the same verified artifact CI uploads. Automated projects exercise the Chromium, Firefox, and WebKit engines. Chromium and WebKit exercise native Web Audio decoding; the verified-pack scenario in the headless Linux Firefox project uses a deterministic in-page Web Audio shim because that runner has no reliable audio sink, while still fetching, byte-limiting, SHA-256-checking, and state-testing the real same-origin MP3 pack. Other Firefox scenarios retain the native browser APIs. These projects do not replace physical Safari, Firefox, iOS, Android, camera, codec, audio-output, assistive-technology, or performance qualification.
